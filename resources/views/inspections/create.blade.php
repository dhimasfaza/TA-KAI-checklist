...
.dropgrid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.drop{border:2px dashed #d1d5db;border-radius:12px;min-height:190px;padding:10px;text-align:center;background:#fafafa;cursor:pointer;position:relative}
.drop:hover{background:#f5f5f5}
.preview{display:none;margin-top:8px;max-height:220px;border:1px solid #e5e7eb;border-radius:10px;width:100%}
...
<div class="container">
  <h1 class="h1">{{ $isEdit ? 'Edit Draft Inspeksi' : 'Form Inspeksi Gudang' }}</h1>
  ...
  <form method="POST"
        action="{{ $isEdit ? route('inspections.update',$inspection) : route('inspections.store') }}"
        ...
        @foreach ($cat->items as $it)
          ...
          <div class="item">
            <div class="grid3">
              ...
              <div class="photos">
                <div class="dropgrid">
                  <div class="drop" onclick="byId('p1-{{ $it->id }}').click()"
                       ondragover="dragOver(event)"
                       ondrop="dropFile(event,'p1-{{ $it->id }}','v1-{{ $it->id }}')">
                    <div>Tarik & lepas / klik untuk pilih <b>Foto 1</b></div>
                    <div class="legend">Contoh: tampak depan</div>
                    <img id="v1-{{ $it->id }}" class="preview"
                      @if($cur && $cur->photo_path_1) src="{{ asset('storage/'.$cur->photo_path_1) }}" style="display:block" @endif>
                    <input id="p1-{{ $it->id }}" type="file" name="items[{{ $it->id }}][photo1]"
                           accept="image/*" style="display:none"
                           onchange="preview(this,'v1-{{ $it->id }}')">
                  </div>

                  <div class="drop" onclick="byId('p2-{{ $it->id }}').click()"
                       ondragover="dragOver(event)"
                       ondrop="dropFile(event,'p2-{{ $it->id }}','v2-{{ $it->id }}')">
                    <div>Tarik & lepas / klik untuk pilih <b>Foto 2</b></div>
                    <div class="legend">Contoh: tampak samping</div>
                    <img id="v2-{{ $it->id }}" class="preview"
                      @if($cur && $cur->photo_path_2) src="{{ asset('storage/'.$cur->photo_path_2) }}" style="display:block" @endif>
                    <input id="p2-{{ $it->id }}" type="file" name="items[{{ $it->id }}][photo2]"
                           accept="image/*" style="display:none"
                           onchange="preview(this,'v2-{{ $it->id }}')">
                  </div>
                </div>

                <div style="margin-top:8px">
                  <label class="label" style="margin-bottom:4px">Foto Tambahan (opsional, bisa pilih banyak)</label>
                  <input type="file" multiple name="items[{{ $it->id }}][extras][]" accept="image/*">
                </div>
              </div>
            </div>
          </div>
...
function dragOver(e){ e.preventDefault(); e.dataTransfer.dropEffect='copy'; }
function dropFile(e,inputId,viewId){
  e.preventDefault();
  const f = e.dataTransfer.files && e.dataTransfer.files[0];
  if(!f) return; const dt = new DataTransfer(); dt.items.add(f);
  const inp = byId(inputId); inp.files = dt.files; preview(inp,viewId);
}
function preview(input,viewId){
  const img = byId(viewId);
